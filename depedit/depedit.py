...
__version__ = "2.3.0.0-modified"

ALIASES = {"form": "text", "upostag": "pos", "xpostag": "cpos", "feats": "morph", "deprel": "func", "deps": "head2",
           "misc": "func2",
           "xpos": "cpos", "upos": "pos"}
    ...
    def __init__(self, transformation_text, line, depedit_container):
        ...
        if instructions is None:
            sys.stderr.write("Depedit says: error in configuration file\n"
                             "Malformed instruction on line " + str(
    ...
    def validate(self):
        ...
        for definition in self.definitions:
            ...
            for criterion in criteria:
                if re.match(
                        r"(text|pos|cpos|lemma|morph|func|head|func2|head2|num|form|upos|upostag|xpos|xpostag|feats|deprel|deps|misc)!?=/[^/=]*/",
                        criterion) is None:
        ...
        for action in self.actions:
            ...
            for command in commands:  # Node action
                if re.match(
                        r"(#[0-9]+>#[0-9]+|#[0-9]+:(func|lemma|text|pos|cpos|morph|head|head2|func2|num|form|upos|upostag|xpos|xpostag|feats|deprel|deps|misc)=[^;]*)$",
                        command) is None:
                    if re.match(r"#S:[A-Za-z_]+=[A-Za-z_\$0-9, ]+$|last$",
                                command) is None:  # Sentence annotation action or quit
                ...
                for token in conll_tokens:
                    if not token.is_super_tok and def_matcher.match(token):
                        if def_matcher.sent_def:
                            if len(node_matches[
                                       def_matcher.def_index]) == 0:  # Only add a sentence anno definition once
                                node_matches[def_matcher.def_index] = [
                ...
                for action in actions:
                    if action == "last":
                        ...
                        if action.startswith("#S:"):  # Sentence annotation instruction
                            ...
                            if "$" in val:
                                group_num_matches = re.findall(r"(\$[0-9]+[LU]?)", val)
                                for g in group_num_matches:
                                    ...
                                    try:
                                        ...
                                        sys.stderr.write(
                                            "The action '" + action + "' refers to a missing regex bracket group '$" +
                                            str(group_num) + "'\n")
                                        sys.exit()
                                    group_str = str(group_num)
                                    val = re.sub(r"\$" + group_str, group_value, val)
                            if key in result[1].sentence.annotations:
                                val0 = result[1].sentence.annotations[key]
                                val = val0 + ";" + val
                            ...
                            if not self.quiet:
                                if result["ID2matcher"][node_position].sent_def:
                                    sys.stdout.write(
                                        "! Warning: Rule is applying a *token* transformation to a *sentence* annotation node:\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--stepwise', action="store_true",
                        help="Output sentence repeatedly after each step (useful for debugging)")
